                ...
                Using doc As New iTextSharp.text.Document(iTextSharp.text.PageSize.LETTER, 36, 36, 36, 36)
                    ...
                    If conceptosNodes IsNot Nothing AndAlso conceptosNodes.Count > 0 Then
                        ...
                        For Each concepto As XmlNode In conceptosNodes
                            table.AddCell(New Phrase(If(concepto.Attributes("ClaveProdServ")?.Value, ""), normalFont))
                            table.AddCell(New Phrase(If(concepto.Attributes("Descripcion")?.Value, ""), normalFont))
                            table.AddCell(New PdfPCell(New Phrase(If(concepto.Attributes("Cantidad")?.Value, ""), normalFont)) With {.HorizontalAlignment = Element.ALIGN_RIGHT})
                            table.AddCell(New PdfPCell(New Phrase(If(concepto.Attributes("ValorUnitario")?.Value, ""), normalFont)) With {.HorizontalAlignment = Element.ALIGN_RIGHT})
                            table.AddCell(New PdfPCell(New Phrase(If(concepto.Attributes("Importe")?.Value, ""), normalFont)) With {.HorizontalAlignment = Element.ALIGN_RIGHT})
                        Next
